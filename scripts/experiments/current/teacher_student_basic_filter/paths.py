...
EXPERIMENT_ROOT = ROOT / "data" / "experiments" / "teacher_student_basic_filter"
...
def assert_experiment_path(path: Path) -> Path:
    ...
    if resolved != root and root not in resolved.parents:
        raise ValueError(f"Refusing to write outside teacher-student experiment root: {resolved}")
    return resolved
