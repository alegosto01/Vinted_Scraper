...
EXPERIMENT_ROOT = ROOT / "data" / "experiments" / "giant_basic_visual"
OFFLINE_RUNS_DIR = EXPERIMENT_ROOT / "offline_runs"
MODELS_DIR = EXPERIMENT_ROOT / "models"
LIVE_SHADOW_DIR = EXPERIMENT_ROOT / "live_shadow"
...
def ensure_experiment_dirs() -> None:
    for path in (EXPERIMENT_ROOT, OFFLINE_RUNS_DIR, MODELS_DIR, LIVE_SHADOW_DIR):
...
def assert_experiment_path(path: Path) -> Path:
    ...
    if resolved != root and root not in resolved.parents:
        raise ValueError(f"Refusing to write outside giant_basic_visual: {resolved}")
    return resolved
