...
from experiments.photo_arbitrage.features import heuristic_bad_photo_probability, local_image_paths_from_row, numeric_series
from experiments.photo_arbitrage.paths import MODEL_CACHE_DIR, ensure_experiment_dirs
...
DEFAULT_METHODS = ("simple", "pyiqa", "aesthetic", "dino")
...
def normalize_methods(raw: str | Iterable[str] | None) -> tuple[str, ...]:
    ...
    if not parts or "all" in parts:
        return DEFAULT_METHODS
    aliases = {"heuristic": "simple", "clip": "aesthetic", "dinov3": "dino"}
    normalized = []
    for part in parts:
...
def add_quality_method_scores(frame: pd.DataFrame, config: MethodConfig | None = None) -> pd.DataFrame:
    ...
    for column in QUALITY_METHOD_COLUMNS:
        if column not in out.columns:
            out[column] = np.nan if column.endswith("Score") or column.endswith("Norm") or column == "DinoEmbeddingDim" else ""
    out["CombinedBadPhotoScore"] = combine_bad_photo_scores(out)
    status_cols = [col for col in ["PyiqaStatus", "AestheticStatus", "DinoStatus"] if col in out.columns]
    if status_cols:
