...
EXPERIMENT_ROOT = ROOT / "data" / "experiments" / "full_scrape_giant_model"
...
def ensure_experiment_dirs() -> None:
    for path in (EXPERIMENT_ROOT, OFFLINE_RUNS_DIR, MODELS_DIR, LIVE_RUNS_DIR, REPORTS_DIR):
...
def assert_experiment_path(path: Path) -> Path:
    ...
    if resolved != root and root not in resolved.parents:
        raise ValueError(f"Refusing to write outside full_scrape_giant_model: {resolved}")
    return resolved
