...
Usage:
    ...
    python scripts/experiments/tracking/backfill.py
"""
...
from experiments.tracking.db import init_db, log_run, log_metrics, run_exists
...
def backfill_deal_finder() -> int:
    count = 0
    base = EXPERIMENTS_ROOT / "deal_finder" / "offline_runs"
    if not base.exists():
        return 0
...
def backfill_full_scrape_model() -> int:
    count = 0
    base = EXPERIMENTS_ROOT / "full_scrape_model" / "offline_runs"
    if not base.exists():
        return 0
    for run_dir in base.iterdir():
        if not run_dir.is_dir():
...
def backfill_benchmark_basic_to_full() -> int:
    count = 0
    base = EXPERIMENTS_ROOT / "benchmark_basic_to_full" / "live_runs"
    if not base.exists():
        return 0
    for run_dir in base.iterdir():
        if not run_dir.is_dir():
...
def backfill_teacher_student() -> int:
    count = 0
    base = EXPERIMENTS_ROOT / "teacher_student_basic_filter" / "offline_runs"
    if not base.exists():
        return 0
    for run_dir in base.iterdir():
        if not run_dir.is_dir():
...
def backfill_photo_arbitrage() -> int:
    count = 0
    base = EXPERIMENTS_ROOT / "photo_arbitrage" / "reports"
    if not base.exists():
        return 0
    for run_dir in base.iterdir():
        if not run_dir.is_dir():
