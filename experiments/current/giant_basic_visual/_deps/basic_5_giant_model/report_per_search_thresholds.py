...
from experiments.current.giant_basic_visual._deps.deal_finder.modeling import TARGET_COL, choose_threshold  # noqa: E402
...
def latest_run_dir() -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No basic_5_giant run with validation scores found under {base}")
    return candidates[0]
...
def write_report(
    ...
    *,
    ...
    lines = [
        "# Basic 5 Giant Model - Per-Search Thresholds",
        "",
    ...
    xgb = per_search[per_search["approach"].eq("xgboost_basic_v1")].sort_values("search_name")
    if not xgb.empty:
        lines.extend(
            [
                "",
                "## XGBoost Per-Search Thresholds",
                "",
    ...
    if not comparison.empty:
        focus = comparison[comparison["approach"].eq("xgboost_basic_v1")].sort_values("search_name")
        if not focus.empty:
            lines.extend(
                [
                    "",
                    "## XGBoost Versus Global Threshold",
                    "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune per-search thresholds for a trained basic_5 giant-model run.")
