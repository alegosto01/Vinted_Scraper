...
from experiments.current.time_to_sell._deps.benchmark_basic_to_full import cascade_runner as cascade
from experiments.current.time_to_sell._deps.deal_finder.dataset import add_identity
from experiments.current.time_to_sell._deps.deal_finder.model_sweep import normalize_image_sources
from experiments.current.time_to_sell._deps.deal_finder.paper_trade_model_benchmark import (
    download_primary_image,
    safe_timestamp_for_path,
)
from experiments.current.time_to_sell._deps.deal_finder.paper_trading import load_search_config_by_folder
from experiments.current.time_to_sell._deps.full_scrape_model.compare_feature_modalities import add_dino_embedding_columns
from experiments.current.time_to_sell._deps.photo_arbitrage.features import add_photo_features
from experiments.current.time_to_sell._deps.photo_arbitrage.quality_methods import (
...
from experiments.current.time_to_sell.paths import (
    EXPERIMENT_ROOT,
    ROOT,
...
class TimestampedStream:
    ...
    def write(self, text: str) -> int:
        ...
        for chunk in text.splitlines(keepends=True):
            if self._line_start:
                self._stream.write(f"[{utc_now_iso()}] ")
...
def install_timestamped_streams() -> None:
    sys.stdout = TimestampedStream(sys.stdout)  # type: ignore[assignment]
    sys.stderr = TimestampedStream(sys.stderr)  # type: ignore[assignment]
...
def resolve_collector_plan(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    plan_rows = cascade.resolve_cascade_plan(
    ...
    return add_new_search_stage1_fallbacks(plan_rows, searches)


def basic_5_giant_model_basename(run_name: str, approach: str) -> str:
    return f"{run_name}_global_basic_5_{approach}_seed42"
...
def basic_5_giant_threshold(run_name: str, approach: str, search_name: str, metadata: dict[str, Any]) -> float:
    ...
    if threshold_path.exists():
        ...
        scoped = table[
            table.get("approach", pd.Series(dtype=object)).astype(str).eq(approach)
            & table.get("search_name", pd.Series(dtype=object)).astype(str).eq(search_name)
        ]
        threshold_col = "per_search_threshold" if "per_search_threshold" in scoped.columns else "threshold"
        if not scoped.empty and threshold_col in scoped.columns:
            value = pd.to_numeric(scoped.iloc[0].get(threshold_col), errors="coerce")
            if pd.notna(value):
                return float(value)
    return float(metadata.get("threshold", 0.5))


def add_new_search_stage1_fallbacks(plan_rows: list[dict[str, Any]], searches: list[str]) -> list[dict[str, Any]]:
    existing = {str(row.get("search_name", "")).lower() for row in plan_rows}
    out = list(plan_rows)
    for search in searches:
        key = str(search).strip().lower()
        if key in existing or key not in NEW_SEARCH_STAGE1_MODELS:
            continue
        approach = NEW_SEARCH_STAGE1_MODELS[key]
        metadata = load_basic_5_giant_metadata(NEW_SEARCH_STAGE1_RUN, approach)
        threshold = basic_5_giant_threshold(NEW_SEARCH_STAGE1_RUN, approach, key, metadata)
        out.append(
            {
                "search_name": key,
                "stage1_source": "basic_5_giant_new_search_fallback",
                "stage1_approach": approach,
                "stage1_threshold": threshold,
                "stage1_metadata": metadata,
                "student_run": NEW_SEARCH_STAGE1_RUN,
                "student_objective": "sold_label_stage1_fallback",
                "student_recall_target": np.nan,
                "student_precision_target": np.nan,
                "student_binary_target": np.nan,
                "stage2_approach": "",
                "stage2_threshold": np.nan,
...
def main() -> int:
    ...
    if args.cmd == "run-loop":
        ...
        print(json.dumps({"event": "run_loop_started", "run_dir": str(out_dir), "at": utc_now_iso()}, sort_keys=True), flush=True)
        while True:
            ...
            time.sleep(float(args.collect_every_hours) * 3600.0)
