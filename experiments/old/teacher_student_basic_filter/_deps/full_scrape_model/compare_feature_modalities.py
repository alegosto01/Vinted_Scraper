...
from experiments.old.teacher_student_basic_filter._deps.deal_finder import model_sweep as base_sweep
from experiments.old.teacher_student_basic_filter._deps.deal_finder.modeling import TARGET_COL, score_with_model
from experiments.old.teacher_student_basic_filter._deps.full_scrape_model.dataset import (
    TASK_SOLD_STATUS,
    available_searches,
    build_search_dataset,
    normalize_id_value,
)
from experiments.old.teacher_student_basic_filter._deps.full_scrape_model.paths import (
    EXPERIMENT_ROOT,
    MODELS_DIR,
    OFFLINE_RUNS_DIR,
    ROOT,
...
def visual_source_path(run_name: str) -> Path:
    return ROOT / "experiments" / "old" / "photo_arbitrage" / "data" / "features" / run_name / "combined_scored.csv"
