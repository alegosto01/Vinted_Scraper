...
from experiments.old.benchmark_basic_to_full._deps.deal_finder.dataset import add_identity
from experiments.old.benchmark_basic_to_full._deps.deal_finder.model_sweep import RulePriceScorer, normalize_image_sources
from experiments.old.benchmark_basic_to_full._deps.deal_finder.modeling import load_pickle, score_with_model
from experiments.old.benchmark_basic_to_full._deps.deal_finder.paper_trade_model_benchmark import (
    download_primary_image,
    safe_timestamp_for_path,
)
from experiments.old.benchmark_basic_to_full._deps.deal_finder.paper_trading import load_search_config_by_folder
from experiments.old.benchmark_basic_to_full._deps.full_scrape_model.compare_feature_modalities import (
    DEFAULT_EXCLUDED_SEARCHES,
    add_dino_embedding_columns,
    add_full_engineered_features,
)
from experiments.old.benchmark_basic_to_full._deps.full_scrape_model.paths import MODELS_DIR as FULL_MODEL_DIR
from experiments.old.benchmark_basic_to_full._deps.photo_arbitrage.features import add_photo_features
from experiments.old.benchmark_basic_to_full._deps.photo_arbitrage.quality_methods import (
...
def best_mode_table(run_name: str) -> pd.DataFrame:
    path = ROOT / "experiments" / "old" / "full_scrape_model" / "data" / "offline_runs" / run_name / "best_by_search_mode.csv"
    if not path.exists():
...
def teacher_student_run_dir(run_name: str) -> Path:
    root = ROOT / "experiments" / "old" / "teacher_student_basic_filter" / "data" / "offline_runs"
    if run_name == "latest":
...
def prepare_for_metadata(frame: pd.DataFrame, metadata: dict[str, Any]) -> pd.DataFrame:
    ...
    if "SearchName" in out.columns:
        search_values = out["SearchName"].fillna("").astype(str).map(
            lambda value: re.sub(r"[^a-z0-9]+", "_", value.strip().lower()).strip("_")
        )
        for col in metadata.get("search_onehot_features", []) or []:
            if col.startswith("search__"):
                out[col] = search_values.eq(col.replace("search__", "", 1)).astype(int)
